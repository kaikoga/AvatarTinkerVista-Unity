...
namespace Silksprite.AvatarTinkerVista.VRChat.Processors
{
    ...
    {
        public static void Process(VRCAvatarDescriptor avatarDescriptor)
        {
            var ativ = avatarDescriptor.GetComponentsInChildren<AtivReduceDynamics>();
            if (ativ.Length == 0)
            {
                return;
            }
            ...
            var keepBoneRoots = ativ
                ...
                .Distinct().ToArray();
            var allPbs = avatarDescriptor.GetComponentsInChildren<VRCPhysBoneBase>()
            ...
            RecordNetworkIdsToSync(avatarDescriptor, keepPbs, GenerateUnusedIds(avatarDescriptor));
            if (reduceOnPlatform)
            {
            ...
            {
                RecordNetworkIdsToSync(avatarDescriptor, reducePbs, GenerateUnusedIdsDescending(avatarDescriptor));
            }
        }
        ...
        static IEnumerable<int> GenerateUnusedIds(VRCAvatarDescriptor avatarDescriptor)
        {
            var networkIdPairs = avatarDescriptor.NetworkIDCollection;
        ...
        static IEnumerable<int> GenerateUnusedIdsDescending(VRCAvatarDescriptor avatarDescriptor)
        {
            var networkIdPairs = avatarDescriptor.NetworkIDCollection;
            var usedIds = networkIdPairs.Select(pair => pair.ID).ToHashSet();
